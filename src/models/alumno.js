...
const getAll = async () => {
  try {
    const [alumnos] = await connection.query(
      'SELECT id_alumno, nombres, apellidos, username, password, rol FROM alumno a JOIN rol r ON a.al_rol = r.id_rol;'
    )
    return alumnos
  } catch (e) {
    console.error(e)
...
const insertOne = async ({ nombres, apellidos, username, hashPass, rol }) => {
  try {
    await connection.execute(
      'INSERT INTO alumno (nombres, apellidos, username, password, al_rol) VALUES (?,?,?,?,?);',
